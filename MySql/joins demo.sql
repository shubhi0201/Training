...
SELECT productCode, productName,textDescription FROM products p INNER JOIN productlines pl ON p.productLine = pl.productLine;
...
SELECT * FROM product WHERE productCode = 'S10_1678';
...
SELECT c.customerNumber, c.customerName, o.orderNumber,  STATUS 
	FROM customers c 
		LEFT JOIN orders o
			 ON c.customerNumber = o.customerNumber IS NULL;

-- Query to display All OrderNumbers, and their matching
-- CustomerNo & Prdct Code of 10123
SELECT o.orderNumber, o.customerNumber, d.productCode
	FROM orders o  
...
SELECT customerNumber, customerName FROM customersclassicmodels
	WHERE customerNumber NOT IN(SELECT DISTINCT customerName FROM orders);
