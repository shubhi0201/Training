...
# MySQL WHERE with the LIKE operator
 ...
 SELECT employeeNumber,lastname,firstname,jobTitle,email,officeCode FROM employees  WHERE firstname LIKE 'A%';
...
-- SELECT, INSERT, UPDATE, or DELETE statement.
 ...
 SELECT employeeNumber,lastname,firstname,jobTitle,email,officeCode FROM employees  WHERE --officeCode IN(1,5,7) ORDER BY officeCode;
...
-- Null -> Absence of Data or Data is missing
 ...
 SELECT employeeNumber,lastname,firstname,jobTitle,email,reportsTo FROM employees  WHERE reportsTo IS  NULL;
...
SELECT jobTitle, firstName FROM employees ORDER BY jobTitle,firstName DESC ;  
...
#  ORDER BY to sort a result set by an expression	
	...
	SELECT orderNumber, orderlinenumber, quantityOrdered, priceEach, quantityOrdered*priceEach   FROM orderdetails ORDER BY  quantityOrdered*priceEach DESC;
...
-- having clouse is used to filter group by clause
